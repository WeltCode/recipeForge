...
import {
  ...
  createFormato, updateFormato, deleteFormato, numTrim, eur, priceLabel,
} from '../../lib/costeo'
import { listSuppliers } from '../../lib/catalog'
import FormatoForm, { fmtToBody, fmtFromStored } from './FormatoForm'
import InsumoPreview from './InsumoPreview'
...
export default function InsumosPanel({ canEdit }) {
  ...
  const [fmtEdit, setFmtEdit] = useState(null)          // format_id en edición inline
  const [saving, setSaving] = useState(false)
  const [preview, setPreview] = useState(null)          // insumo a previsualizar
  ...
  const openNew = () => { setForm(EMPTY_INS); setExistingFormats([]); setExistingRef(null); setPending([]); setFmtEdit(null); setEditing('new') }
  const openEdit = (i) => {
    setForm({ name: i.name })
    setExistingFormats(i.formats || []); setExistingRef(i.reference_format); setPending([]); setFmtEdit(null)
    setEditing(i.id)
  }
  const close = () => { setEditing(null); setForm(EMPTY_INS); setExistingFormats([]); setPending([]); setFmtEdit(null) }
  ...
  const saveFmtEdit = async (id, f) => { try { await updateFormato(id, fmtToBody(f)); setFmtEdit(null); reload() } catch (e) { setError(e.message) } }
  const delExisting = async (id) => { if (!window.confirm('¿Quitar este formato de compra?')) return; try { await deleteFormato(id); reload() } catch (e) { setError(e.message) } }
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const p of pending) await createFormato({ insumo: insumo.id, ...p })
      close(); load()
  ...
  // Panel de edición/creación (se muestra en línea bajo la fila, no en el tope).
  const editPanel = (isNew) => (
    <div className="rounded-xl border border-steel-300 bg-steel-50/50 p-4">
      {isNew && (
        <div className="mb-3 flex items-center justify-between">
          <h3 className="pass-title text-[15px] text-ink">Nuevo insumo</h3>
      ...
      <div className="mt-4 rounded-lg border border-steel-300 bg-white/70 p-3">
        <p className="pass-title mb-2 text-[13px] text-ink">Añadir formato de compra</p>
        ...
        {(existingFormats.length || pending.length) ? (
          <div className="mt-3 overflow-hidden rounded-lg border border-steel-200">
            {existingFormats.map((f, idx) => {
              ...
              return (
                <div key={f.id} className={`bg-white ${idx ? 'border-t border-steel-200' : ''}`}>
                  {fmtEdit === f.id ? (
                    <div className="p-3">
                      <p className="mb-2 text-[12px] font-medium text-ink">Editar formato</p>
                      <FormatoForm key={`fe-${f.id}`} suppliers={suppliers} initial={fmtFromStored({ ...f, supplier: f.supplier })}
                        submitLabel="Guardar" onCancel={() => setFmtEdit(null)} onAdd={(nf) => saveFmtEdit(f.id, nf)} />
                    </div>
                  ) : (
                    <div className="flex flex-wrap items-center gap-2 px-3 py-2">
                      ...
                      {!isRef && <button onClick={() => setReference(f.id)} title="Usar como precio de referencia" className="rounded-lg steel-plate px-2 py-1 text-[11px] font-medium text-ink hover:bg-white">Usar</button>}
                      <button onClick={() => setFmtEdit(f.id)} title="Editar formato" className="rounded-lg steel-plate px-2 py-1 text-[11px] font-medium text-ink hover:bg-white">Editar</button>
                      <button onClick={() => delExisting(f.id)} title="Quitar" className="text-ink-3 hover:text-danger"><Trash size={14} /></button>
                    </div>
                  )}
                </div>
              )
          ...
          {rows.map((i, idx) => (
            <div key={i.id} className={idx ? 'border-t border-steel-200' : ''}>
              <div className="flex items-center gap-4 px-4 py-3.5 sm:px-5">
                ...
                {canEdit && <>
                  <button onClick={() => (editing === i.id ? close() : openEdit(i))} title="Editar" className={`grid h-9 w-9 place-items-center rounded-lg hover:bg-steel-100 ${editing === i.id ? 'bg-steel-100 text-ink' : 'text-ink-3 hover:text-ink'}`}><Pencil size={16} /></button>
              ...
              {editing === i.id && <div className="border-t border-steel-200 bg-steel-50/40 px-4 py-4 sm:px-5">{editPanel(false)}</div>}
            </div>
          ))}
        </div>
      ) : (
